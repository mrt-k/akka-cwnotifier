...
object Server {
  def main(args: Array[String]) {
    ...
    // 自身のroom id $ export MROOM_ID=YOUR_ROOM_ID
    val MY_ROOM_ID = try{ sys.env("MY_ROOM_ID").toString } catch { case e:Exception => println("MY_ROOM_ID has not been set"); sys.exit(-1)}
    // 送信先のroom_id
    val ROOM_IDS: Array[String] = try{ sys.env("ROOM_IDS") split ":" } catch { case e: Exception => println("ROOM_ID has not been set"); sys.exit(-1)}
      ...
      path("") {
        get {
          complete {
            ...
            notifier.notifier(API_KEY, MY_ROOM_ID)
            notifier.room_notifier(API_KEY, ROOM_IDS)
            "Send message!"
          }
        }
      }
